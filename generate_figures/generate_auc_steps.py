...
# Donnees du tableau steps sensitivity k=3
...
ax.errorbar(steps, aucs, yerr=sems,
            ...
            linestyle="-", label="BNT-MAML-TTA (k=3)", zorder=4)
ax.fill_between(steps,
                [v-e for v,e in zip(aucs, sems)],
                [v+e for v,e in zip(aucs, sems)],
                color="#C0392B", alpha=0.12)

# Marquer steps=3 comme optimal
...
plt.tight_layout()
plt.savefig("figures/fig_auc_steps.pdf",
            dpi=300, bbox_inches="tight")
plt.savefig("figures/fig_auc_steps.png",
            dpi=300, bbox_inches="tight")
