...
class MAMLGraphTransformer(nn.Module):
    def __init__(self, backbone):
        super().__init__()
        self.attention_list = backbone.attention_list
        self.readout        = backbone.readout
        self.fc_features    = backbone.fc[:4]  # Linear(200,800) + LeakyReLU + Linear(800,256) + LeakyReLU
        ...
        # Copier readout specifique
        if hasattr(backbone, 'dim_reduction'):
            self.dim_reduction = backbone.dim_reduction
        if hasattr(backbone, 'norm'):
            self.norm = backbone.norm
        self.node_num = backbone.node_num
...
BASE_CFG = OmegaConf.create({
    ...
    "model": {"name": "GraphTransformer", "readout": "mean", "self_attention_layer": 2},
...
def main(inner_lr=0.001, k_shot=3, num_inner_steps=3):
    ...
    for run_idx, seed in enumerate(seeds):
        ...
        with open_dict(cfg):
            ...
            cfg.k_shot          = k_shot
            cfg.unique_id       = "gt_maml_run{}_seed{}".format(run_idx, seed)
            cfg.total_steps     = cfg.training.epochs
            cfg.steps_per_epoch = 1

        gt_cfg = OmegaConf.create({
            "dataset": {"node_sz": 200, "node_feature_sz": 200, "timeseries_sz": 200},
            "model": {"readout": "mean", "self_attention_layer": 2}
        })
        backbone = GraphTransformer(gt_cfg).cuda()
        pretrained_path = (
            "result_graphtransformer/GT_run{}_seed{}/".format(run_idx, seed) +
            "model_gt_run{}_seed{}.pt".format(run_idx, seed)
        )
        backbone.load_state_dict(torch.load(pretrained_path, map_location="cuda"))
        print("MLP charge : {}".format(pretrained_path))

        model = MAMLGraphTransformer(backbone).cuda()
    ...
    sep = "=" * 60
    print("\n" + sep + "\nRESUME GraphTransformer + MAML + TTA\n" + sep)
    print("\nINTRA-SITE :")
